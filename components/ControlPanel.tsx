  ...
  const SegmentButton = ({ label, active, onClick, extraClass = "", tooltipText }: { label: string, active: boolean, onClick: () => void, extraClass?: string, tooltipText: string }) => (
    <button 
        ...
        className={`flex-1 py-2 text-xs font-semibold rounded-lg transition-all duration-300 ease-[cubic-bezier(0.25,0.8,0.25,1)] ${
            active 
            ? 'bg-morandi-text text-white shadow-soft transform scale-100 font-sans' 
            : 'text-morandi-overlay hover:bg-morandi-surface0 hover:text-morandi-text font-sans'
        } ${extraClass}`}
    >
        {label}
    </button>
    ...
    <div className="flex flex-col h-full glass-panel w-96 p-8 overflow-y-auto rounded-r-3xl border-r border-white/60 shadow-glass z-10 relative">
      ...
      <div className="mb-10 flex items-center justify-between">
        <div className="flex items-center gap-3">
            ...
            <div>
                <h1 className="text-xl font-serif italic font-bold text-morandi-text tracking-tight">
                NeuroRestore
                </h1>
                <p className="text-[10px] text-morandi-subtext font-medium tracking-wide uppercase font-sans">PDSR Engine 3.0</p>
            </div>
        </div>
        {onChangeKey && (
            <button 
      ...
      <div className="space-y-8">
        ...
        <div className="space-y-6 animate-fade-in">
            ...
            {(mode === AppMode.RESTORATION) && (
                <div className="p-5 bg-morandi-blue/5 rounded-2xl border border-morandi-blue/10 shadow-soft space-y-4">
                     <h3 className="text-sm font-serif italic font-bold text-morandi-blue tracking-wide flex items-center gap-2">
                     ...
                     <div className="space-y-3">
                         ...
                         >
                            <span className="text-[11px] font-sans font-medium text-morandi-text group-hover:text-morandi-blue transition-colors">Text-Prior Injection</span>
                         ...
                         >
                            <span className="text-[11px] font-sans font-medium text-morandi-text group-hover:text-morandi-blue transition-colors">Neural Texture Transfer</span>
                         ...
                         >
                            <span className="text-[11px] font-sans font-medium text-morandi-text group-hover:text-morandi-blue transition-colors">Semantic Damage Repair</span>
            ...
            {(mode === AppMode.VECTORIZATION) && (
                <div className="p-5 bg-morandi-green/5 rounded-2xl border border-morandi-green/10 shadow-soft space-y-4">
                     <h3 className="text-sm font-serif italic font-bold text-morandi-green tracking-wide flex items-center gap-2">
                     ...
                     <div className="space-y-3">
                         ...
                         >
                            <span className="text-[11px] font-sans font-medium text-morandi-text group-hover:text-morandi-green transition-colors">DiffVG Topology Opt.</span>
                         ...
                         >
                            <span className="text-[11px] font-sans font-medium text-morandi-text group-hover:text-morandi-green transition-colors">Material Physics</span>
                     ...
                     <div className="pt-2 border-t border-morandi-green/20">
                        <label className="text-[10px] font-bold text-morandi-subtext mb-1 block font-sans">Vector Detail</label>
                        <div className="flex bg-white/50 rounded-lg p-1">
                            {['LOW', 'MEDIUM', 'HIGH'].map((level) => (
                                <button
                                    key={level}
                                    onClick={() => handleChange('vectorDetail', level)}
                                    className={`flex-1 py-1 text-[9px] font-bold rounded transition-all font-sans ${config.vectorDetail === level ? 'bg-morandi-green text-white shadow-sm' : 'text-morandi-overlay hover:text-morandi-green'}`}
                                >
                                    {level}
                                </button>
                            ))}
            ...
            <div className="space-y-4">
                 <div className="flex justify-between items-center mb-1">
                     <h3 className="text-[10px] font-bold text-morandi-overlay uppercase tracking-wider font-sans">Output Quality</h3>
                 </div>
                 <div className="grid grid-cols-3 gap-2">
                    {Object.values(Resolution).map((res) => (
                        <button
                            ...
                            className={`py-2 rounded-lg text-[10px] font-bold border transition-all font-sans ${
            ...
            {/* Custom Prompt Input */}
            <div>
                 <h3 className="text-[10px] font-bold text-morandi-overlay uppercase tracking-wider mb-2 font-sans">
                 ...
                 <textarea 
                    ...
                    className="w-full h-24 p-3 bg-white/60 rounded-xl border border-morandi-surface1 text-xs text-morandi-text focus:outline-none focus:bg-white focus:border-morandi-blue focus:ring-1 focus:ring-morandi-blue transition-all resize-none placeholder:text-morandi-overlay/70 font-sans"
                 />
            </div>
        ...
        <div className="pt-4 border-t border-morandi-surface0">
             <button 
                ...
                className={`w-full py-4 rounded-xl text-sm font-bold tracking-wide shadow-lg transition-all transform hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-2 font-sans ${
             ...
             <div className="mt-3 h-4 flex items-center justify-center">
                 {tooltip && (
                     <span className="text-[10px] text-morandi-blue font-medium animate-fade-in font-sans">{tooltip}</span>
                 )}
             </div>
        </div>
